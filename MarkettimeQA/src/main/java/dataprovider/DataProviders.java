...
//Class --> AccountCreationPage  Test Case--> verifyCreateAccountPageTest	
	...
	@DataProvider(name = "newAccountCreations")
	public Object[][] accountCreation() {

		// Totals rows count
		int rows = obj.getRowCount("AccountCreations");
		// Total Columns
		int column = obj.getColumnCount("AccountCreations");
		int actRows = rows - 1;
		...
		for (int i = 0; i < actRows; i++) {
			...
			for (int j = 0; j < column; j++) {
				hashMap.put(obj.getCellData("AccountCreations", j, 1),
						obj.getCellData("AccountCreations", j, i + 2));
			}
			data[i][0] = hashMap;
		}
		return data;
